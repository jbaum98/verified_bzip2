...
preceding chracter, and then sorting by the character 2 spaces in
front.
...
However, the column direclty to the left of the sorted column is very
...
because those are the characters that preced \texttt{a}s in the
...
This requires that the \texttt{d} preced the \texttt{!} in the last
column.
...
We can also think of this as computing the fixpoint of $\var{hdsort} \circ
...
\begin{proof}
  ...
  \begin{equation*}
    \var{radixsort}~m \coloneqq (\var{hdsort} \circ \var{map}~\var{rrot})^n~m
  \end{equation*}
  where $m$ is a matrix with $n$ columns. In this case, we are sorting
  the rotation matrix and we let $n = \var{length}~l$. Then we have
  \begin{align*}
       &(\var{hdsort} \circ \var{map}~\var{rrot})~(\var{lexsort}~(\var{rots}~l)) \\
    ={}&(\var{hdsort} \circ \var{map}~\var{rrot})~((\var{hdsort} \circ \var{map}~\var{rrot})^n~(\var{rots}~l)) \\
    ={}&(\var{hdsort} \circ \var{map}~\var{rrot})^{n+1}~(\var{rots}~l) \\
    ={}&(\var{hdsort} \circ \var{map}~\var{rrot})^n~((\var{hdsort} \circ \var{map}~\var{rrot})~(\var{rots}~l)) \\
    ={}&\var{lexsort}~((\var{hdsort} \circ \var{map}~\var{rrot})~(\var{rots}~l)) \\
    ={}&\var{lexsort}~((\var{hdsort} \circ \var{rrot})~(\var{rots}~l)) \\
    ={}&\var{lexsort}~(\var{rots}~l)
  \end{align*}
  The last step follows because the function $\var{hdsort} \circ
...
={}& \var{recreate}~j~(\var{bwp}~l) \qedhere
\end{align*}
\end{proof}
...
\begin{proof}
  ...
  sorted, we knnow that $h \le h'$ and $h' \le h$, implying $h \equiv h'$.
...
\begin{corollary}[StableSort\_unique]
  For all functions $f, g: \var{list}~A \to \var{list}~A$,
  \begin{equation*}
...
Although it is difficult to prove, it is intuitive that prefix orders
that can look at more elements of the lists are ``stronger'' than
those that can look at fewer.
...
\begin{equation*}
  \var{radixsort} = (\var{hdsort} \circ \var{map}~\var{rrot})^n
\end{equation*}
Or, in Coq:
\lstinputlisting[
...
permtuation of simply rotating the last column to the front $j$
times.
\begin{lemma}[radixsort\_perm\_inv]
  For all matrices $m$ and $j : \mathbb{N}$,
  \begin{equation*}
    \var{Permutation}~((\var{map}~\var{rrot})^j~m)~
                      ((\var{hdsort} \circ \var{map}~\var{rrot})^j~m)
  \end{equation*}
\end{lemma}
\begin{proof}
  We reason by induction on $j$. In the inductive case,
  \begin{align*}
    (\var{hdsort} \circ \var{map}~\var{rrot})^{j+1}~m ={}&
    \var{hdsort}~(\var{map}~\var{rrot}~(\var{hdsort} \circ
    \var{map}~\var{rrot})^j~m) \\
    \eqperm{}& \var{map}~\var{rrot}~((\var{hdsort} \circ
    \var{map}~\var{rrot})^j~m) \\
...
\begin{proof}
  \begin{align*}
    ...
    ={}& (\var{map}~\var{rrot})^n~m \\
    \eqperm{}& (\var{hdsort} \circ \var{map}~\var{rrot})^n~m \\
...
\begin{proof}
  ...
  \begin{align*}
    ...
    \var{hdsort}~(\var{map}~\var{rrot}~(\var{hdsort} \circ
    \var{map}~\var{rrot})^j~m) \\
    \eqstable{}& \var{map}~\var{rrot}~((\var{hdsort} \circ
    \var{map}~\var{rrot})^j~m) \\
...
that it uses insertion sort as the innner sort, but
