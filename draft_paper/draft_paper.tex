...
Even though both are sorted on the first character, because \var{fog}
...
Most radix sorts rely on a some stable sorting subroutine.
...
about sorting algorithms whose implementation is irrelevent as long as
...
hypotheses, especially in the transitive case. Insetad, we reason
about specifically insertion sort. Therefore
\cref{thm:stablesort_unique} is quite useful, because we can reason
using insertion sort, and later on replace it with any other
...
\var{list}~\nat$ represents a permutation function on $n$ elements.
...
sorts must preserve the relative orders of ``equivalent'' elements.
This equivalence comes from the ordering we are using: any total
decidable preorder also give us a decideable equivalence relation $\equiv$,
where $x \equiv y$ when $x \le y$ and $x \ge y$.
...
This can often be extended even to unstable sorting algorithms.
Remember, it isn't always true that $x \equiv y$ implies $x = y$. For
example, if we are sorting strings by their first character, then
$\text{\texttt{fit}} \equiv \text{\texttt{fog}}$, but clearly
$\text{\texttt{fit}} \neq \text{\texttt{fog}}$. When we do have that the
equivalence ($\equiv$) implies Leibniz equality ($=$), we say that we have
an \textit{order}, as we have when we are sorting strings using all of
their characters. Under an order, as opposed to a preorder, equivalent
elements are actually indistinguishable, so you can't tell if they
have been swapped.
...
define \var{rrot}, which moves the last element of a list to the
front, and relies on \var{init}, which drops the last element of a
list. Note that in order to ensure that $\var{last}$ is total, it
takes an extra argument to return in the case of an empty list.
\begin{lstlisting}
...
stable permutation of the original matrix had we only rotated $j$
times without sorting.
...
Specifically, if we examine how an LSD radix sort sorts the rotation
...
move with it for similar reasons. The column directly to the right of
the shaded column is the result of sorting the input string, and then
stably sorting it again by the character that precedes it in the
original string. The next column is the result of sorting by each
character, sorting by the preceding character, and then sorting by the
character 2 spaces in front.
...
\end{equation*}
where $\var{prefix\_shift}~c$ adds $c$ as the first column of a
matrix, dropping the last column, and $m$ is the rotation matrix. This
means as we repeatedly prefix $l$ and sort, we preserve the ``rest of
the matrix'', even the parts that we have yet to reconstruct.
...
More precisely, suppose we had a function \var{recreate}
such that
\begin{equation}
  \label{eq:recreate}
...
before. To do so, we need to prove a few other theorems.
...
\begin{lemma}[lexsort\_rots\_hdsort]
  For all lists $l$,
  \begin{equation*}
    (\var{hdsort} \comp
...
We can define the preimage of an index under a permutation function.
\begin{lstlisting}
